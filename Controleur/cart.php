<?php  
	...
	*/
	...
	function addNewArticlesToCart($articles) {
		$cart =& findcart();
		$add = array('qty' => 1, 'articles' => $articles);
		$cart['lineitems'][] = $add;
	}

	/*
		Ajoute un article au panier. Cherche dans le panier courant si l'article existe.
		Si oui alors augmente la qty commandée de 1 pour ce livre. 
	...
	*/
	...
	/*
		...
		Si le livre est déjà présent dans le panier alors nous augmentons de un la qty.
	...
	*/
	...
	function addToCart($articlesId, $connect) {
		...
		include_once("Controleur/afficher_article_panier.php");
		$articles = getArticle($db,1, $articlesId);
	...
	*/
	...
	function cartTotalPrice() {
		...
		foreach ($cart['lineitems'] as $key => $item) {
			$total += $item['articles']['prix'] * $item['qty'];
		}
		return $total;
	}
